...
def main(args):
    ...
    with graph.as_default():
        ...
        #######################################################
            ...
            # head_variables = tf.contrib.framework.get_variables_to_restore(include=[HEAD_SCOPE])
        ...
        with tf.Session(graph=graph) as sess:
            ...
            print("Restoring network variables...")
            # sess.run(init_head) # head variables
            head_saver.restore(sess, args.save_path)
            ...
            for epoch in range(args.num_epochs1):
                ...
                while True:
                    try:
                        loss = sess.run([total_loss], {is_training: True})
